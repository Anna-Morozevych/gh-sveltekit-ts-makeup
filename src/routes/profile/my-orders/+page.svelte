<script lang="ts">
  ...
  onMount(() => {
    ...
    getDocs((ordersRef))
     .then(res => {
      let ordersFromServer: Order[] = [];

      res.docs.forEach((doc) => {
        if (doc.data().userId === currentUserid) {
          const total = +doc.data().orderInfo.reduce((sum: number, el: Product) => sum + (+el.price), 0).toFixed(2)
  ...
  <div class="orders">
    ...
    <div class="orders__order">
      <div class="orders__info">
        ...
        <p class="orders__status" style="background-color: {statusList.find(el=> el.name === order.status)?.color};">
          {order.status}
        </p>
      </div>
      ...
      {#each order.orderInfo as product}
        <div class="product">
          ...
          <div class="product__details">
            <a class="product__link" href="{base}/categories/{product.product_type}/{product.id}">{product.name}</a>
